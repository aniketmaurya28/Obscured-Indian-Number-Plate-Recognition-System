"""
=================================================================
ANPR SYSTEM v12.3 — CRITICAL FIXES APPLIED
=================================================================
FIXES IN v12.3 (vs v12.2):
  FIX-1: Pipeline step order corrected
         Camera enhance → Low light → Dirty detect → Dirty clean
         → Perspective correct → Line detect → OCR
  FIX-2: PlateLineDetector thresholds tightened
         GAP_THRESHOLD: 0.15 → 0.08
         MIN_GAP_ROWS:  3    → 6
         NEW: MIN_GAP_HEIGHT_RATIO = 0.08 (contiguous gap check)
  FIX-3: TwoLinePlateHandler._read_half() no longer calls
         PerspectiveCorrector on half-crops (uses _deskew only)
  FIX-4: VotingSystem._vote() length selection now validity-biased
         RegexValidator check adds bonus to valid-format lengths
  FIX-5: ConfusionResolver._fix_format() threshold replaced
         bs > len(text) → bs > 0 and bs > baseline_score
  FIX-6: MultiPreprocessor.all_variants() skips duplicate
         dirty_binary when dirty_cleaned is already binary
...
# FIX-2: Tightened thresholds + contiguous gap check
# ══════════════════════════════════════════
class PlateLineDetector:
    ...
    def is_two_line(bgr):
        ...
        min_gap_pixels = int(h * PlateLineDetector.MIN_GAP_HEIGHT_RATIO)
        contiguous_gap = 0
        max_contiguous = 0
        for val in mid:
            if val < peak * PlateLineDetector.GAP_THRESHOLD:
                contiguous_gap += 1
                max_contiguous = max(max_contiguous, contiguous_gap)
            else:
                contiguous_gap = 0

        has_gap = (len(gap_rows) >= PlateLineDetector.MIN_GAP_ROWS
                   and max_contiguous >= min_gap_pixels)

        print(f"  [LINE-DETECT] peak={peak:.3f} gap_rows={len(gap_rows)} "
              f"max_contiguous={max_contiguous} min_needed={min_gap_pixels} "
              f"two_line={has_gap}")
        return has_gap


# ══════════════════════════════════════════
# TWO-LINE PLATE HANDLER
# FIX-3: _read_half no longer calls PerspectiveCorrector
# ══════════════════════════════════════════
class TwoLinePlateHandler:
    ...
    def _read_half(crop, easy_engine, paddle_engine):
        ...
        try:
            deskewed = PerspectiveCorrector._deskew(crop)
            if (deskewed is not None and
                    deskewed.shape[0] > 0 and deskewed.shape[1] > 0):
                deskew_variants = MultiPreprocessor.all_variants(deskewed)
            else:
                deskew_variants = {}
        except Exception:
            deskew_variants = {}
        ...
        if paddle_engine and paddle_engine.loaded:
            ...
            if deskew_variants:
                paddle_results += paddle_engine.read(deskew_variants)
        if easy_engine:
            easy_results += easy_engine.read(variants)
            if deskew_variants:
                easy_results += easy_engine.read(deskew_variants)
...
# ══════════════════════════════════════════
# STEP 4 — MULTI-PREPROCESSING
# FIX-6: dirty_binary skipped when input is already binary
# ══════════════════════════════════════════
class MultiPreprocessor:
    ...
    def all_variants(bgr, dirty_cleaned=None):
        ...
        if dirty_cleaned is not None:
            ...
            variants['dirty']      = ds
            variants['dirty_grey'] = cv2.cvtColor(dgc, cv2.COLOR_GRAY2BGR)

            colour_std = float(np.std(ds))
            is_already_binary = colour_std < 10.0
            if not is_already_binary:
                variants['dirty_binary'] = cv2.cvtColor(dotsu,
                                               cv2.COLOR_GRAY2BGR)
            else:
                k_sharp = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])
                variants['dirty_sharp'] = np.clip(
                    cv2.filter2D(ds, -1, k_sharp), 0, 255).astype(np.uint8)
...
# FIX-4: Length selection is now validity-biased
# ══════════════════════════════════════════
class VotingSystem:
    ...
    def _vote(reads):
        ...
        def length_score(k):
            items      = by_len[k]
            count      = len(items)
            total_conf = sum(c for _, c, _ in items)
            avg_conf   = total_conf / max(count, 1)
            valid_bonus = 0
            for t, c, _ in items:
                is_valid, _, _ = RegexValidator.validate(t)
                if is_valid:
                    valid_bonus = 8
                    break
            return count * 2 + avg_conf * 4 + valid_bonus

        best_len = max(by_len, key=length_score)
...
# FIX-5: _fix_format threshold replaced
# ══════════════════════════════════════════
    ...
    def _fix_format(text):
        ...
        for fmt in ConfusionResolver.FORMATS:
            if len(fmt) != len(text):
                continue
            s = 0
            for ch, exp in zip(text, fmt):
                if exp == 'L':
                    if ch.isalpha():
                        s += 3
                    elif ch.isdigit():
                        s += 0 if ConfusionResolver.D2L.get(ch) else -2
                elif exp == 'D':
                    if ch.isdigit():
                        s += 3
                    elif ch.isalpha():
                        if ch in ConfusionResolver.PROTECTED_FROM_DIGIT:
                            ...
                            s += 0 if ConfusionResolver.L2D.get(ch) else -2
            baseline_score = max(baseline_score, s)
        ...
        accept = bs > 0 and bs > baseline_score
        return (best, bc) if accept else (text, [])
...
# ══════════════════════════════════════════
# MAIN PIPELINE — v12.3 CORRECTED ORDER
# FIX-1: Camera enhance → Low light → Dirty detect+clean
#         → Perspective correct → Line detect → OCR
# ══════════════════════════════════════════
class ANPRPipeline:
    ...
    def run(self, plate_bgr, is_camera=False):
        ...
        dirty_score, dirty_severity, dirty_breakdown = \
            DirtyImageDetector.score(plate_bgr)
        dirty_cleaned  = None
        cleaning_steps = []
        if dirty_severity != 'CLEAN':
            dirty_cleaned, cleaning_steps = \
                DirtyImageCleaner.clean(plate_bgr, dirty_severity)

        pre_corrected = dirty_cleaned if dirty_cleaned is not None else plate_bgr

        corrected = PerspectiveCorrector.correct(pre_corrected)
...
class CustomANPR:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(
            "ANPR v12.3 | Critical Fixes: Pipeline Order + Two-Line Detection + Voting"
        )
    ...
    def setup_ui(self):
        ...
        Label(
            hdr,
            text="  ANPR v12.3  |  FIX: Pipeline Order + Two-Line + Voting + Format Resolver",
            bg="#0a1628", fg="white",
        ...
        Label(
            box,
            text="FINAL OUTPUT — v12.3 FIXES APPLIED",
            bg="#0b1a0b", fg="#69F0AE",
            font=("Segoe UI", 9, "bold")
    ...
    def load_ai(self):
        try:
            self._sub("Loading YOLOv8...")
            self.yolo = YOLO(MODEL_PATH)
            print("[OK] YOLO")

            self._sub("Loading PaddleOCR...")
            paddle_ok = self.paddle_engine.load()

            self._sub("Loading EasyOCR...")
            try:
                import easyocr
    ...
    def _ready(self, paddle_ok):
        ...
        parts += ["v12.3-FIXES"]
        self.lbl_status.config(text="ANPR v12.3 — Ready", fg="#00E676")
