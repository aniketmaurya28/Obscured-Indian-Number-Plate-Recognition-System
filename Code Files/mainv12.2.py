...
Pipeline (exact flowchart order):
  ...
  1.5  Dirty Detection   - score: blur/contrast/colour/edges
  1.6  Dirty Cleaning    - morph open + NLM + CLAHE + adaptive
  2.   YOLOv8            - detect plate (multi-scale retry +
                          contour fallback if YOLO fails)
  3.   Perspective       - correct skew / tilt
  2.5  Line Detection    - PlateLineDetector splits pipeline:
         Single-line  →  Steps 4-9 (standard route)
         Two-line     →  TwoLinePlateHandler (split + merge)
  4.   Multi-Preprocess  - Plain/Grey/Binary/CLAHE/Sharp/Night/
                          Bloom/Dirty (when dirty cleaning ran)
  5.   PaddleOCR + EasyOCR - dual engine reading
  6.   Voting System     - weighted confidence merge
  7.   65-Pair Resolver  - confusion correction
  8.   Regex Validation  - Indian plate format check
  9.   Final Output      - display + CSV save

NEW IN v12.2:
  - PlateLineDetector: horizontal projection profile detects
    genuine two-line plates from pixel content (not aspect ratio)
  - TwoLinePlateHandler: RESTORED with safer content-based trigger
    splits crop into top/bottom halves, reads independently,
    merges result through ConfusionResolver + Regex
  - DirtyImageDetector (v12.0): composite score from 4 metrics
  - DirtyImageCleaner (v12.0): four-stage cleaning pipeline
  - New 'dirty' variants in MultiPreprocessor
  - Multi-scale YOLO retry: 1.0x → 1.5x → 2.0x
  - Contour fallback: when YOLO fails at all scales, OpenCV
    rectangle detection finds plate-like regions

INHERITED FROM v11.9:
  - RESTORED ConfusionResolver from v11.3
  - PROTECTED_FROM_DIGIT: Removed C, H, S
  - _get_protected_positions: Restored Pattern 2
  - _fix_format: Penalty -1, _fix_weak: Window ±2 threshold 3
...
# ══════════════════════════════════════════
# STEP 1.5 — DIRTY IMAGE DETECTOR
# ══════════════════════════════════════════
class DirtyImageDetector:
    """
    Scores a plate crop across four metrics and returns
    a composite 0-1 dirtiness score plus a severity level.

    Metrics
    -------
    1. Laplacian variance  — low = blurry/soft (mud/frost)
    2. Contrast std dev    — low = flat low-contrast image
    3. Colour anomaly      — high = dominant non-white/yellow hue
    4. Edge density        — low = few character edges visible

    Severity
    --------
    CLEAN   score < 0.35   no cleaning needed
    MILD    score < 0.55   light clean (denoise + CLAHE)
    SEVERE  score >= 0.55  full clean pipeline
    """
    ...
    def score(bgr):
        ...
        lap_score = 1.0 - np.clip(
            ...
            max(DirtyImageDetector.LAP_SHARP_MIN -
                DirtyImageDetector.LAP_BLUR_MAX, 1),
            0.0, 1.0)

        std_dev        = float(np.std(gray))
        contrast_score = 1.0 - np.clip(
            (std_dev - DirtyImageDetector.CONTRAST_POOR) /
            max(DirtyImageDetector.CONTRAST_GOOD -
                DirtyImageDetector.CONTRAST_POOR, 1),
            0.0, 1.0)

        hsv  = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(hsv)
        dirt_mask = (s > 30) & ~((h > 15) & (h < 35))
        colour_score = np.clip(
            float(np.sum(dirt_mask)) / max(gray.size, 1) * 3.5,
            0.0, 1.0)
        ...
        edge_score = 1.0 - np.clip(
            ...
            max(DirtyImageDetector.EDGE_GOOD -
                DirtyImageDetector.EDGE_POOR, 0.001),
            0.0, 1.0)
...
# ══════════════════════════════════════════
# STEP 1.6 — DIRTY IMAGE CLEANER
# ══════════════════════════════════════════
class DirtyImageCleaner:
    """
    Four-stage cleaning based on severity.
    MILD   → stages 2+3 (denoise + CLAHE)
    SEVERE → all 4     (morph open + denoise + CLAHE + adaptive)
    """
    ...
    def clean(bgr, severity):
        ...
        if severity == 'SEVERE':
            k = cv2.getStructuringElement(
                cv2.MORPH_RECT,
                (DirtyImageCleaner.MORPH_KERNEL,
                 DirtyImageCleaner.MORPH_KERNEL))
        ...
        l, a, b   = cv2.split(lab)
        l_eq      = cv2.createCLAHE(clipLimit=clip,
                                     tileGridSize=(8, 8)).apply(l)
        result    = cv2.cvtColor(cv2.merge([l_eq, a, b]),
                                 cv2.COLOR_LAB2BGR)
        steps.append('clahe')
...
# ══════════════════════════════════════════
# STEP 2.5 — PLATE LINE DETECTOR
# ══════════════════════════════════════════
class PlateLineDetector:
    """
    Uses horizontal projection profile to detect two-line plates.

    How it works
    ------------
    - Binarise plate crop (black text on white)
    - Sum dark pixels per row → projection profile
    - If a clear low-density gap exists near the vertical centre,
      it is a two-line plate
    - Single-line plates have no such gap
    """

    SEARCH_TOP    = 0.30   # only search middle 30%-70% of height
    SEARCH_BOTTOM = 0.70
    GAP_THRESHOLD = 0.15   # row is a gap if density < 15% of peak
    MIN_GAP_ROWS  = 3      # gap must span at least 3 rows
    ...
    def is_two_line(bgr):
        ...
        h, w   = binary.shape
        proj   = np.sum(binary > 0, axis=1).astype(float) / max(w, 1)

        top    = int(h * PlateLineDetector.SEARCH_TOP)
        bot    = int(h * PlateLineDetector.SEARCH_BOTTOM)
        mid    = proj[top:bot]

        if len(mid) == 0:
            return False

        peak = float(np.max(proj))
        if peak == 0:
            return False

        gap_rows = np.where(
            mid < peak * PlateLineDetector.GAP_THRESHOLD)[0]
        has_gap  = len(gap_rows) >= PlateLineDetector.MIN_GAP_ROWS

        print(f"  [LINE-DETECT] peak={peak:.3f} "
              f"gap_rows={len(gap_rows)} two_line={has_gap}")
        return has_gap


# ══════════════════════════════════════════
# TWO-LINE PLATE HANDLER (RESTORED v12.2)
# ══════════════════════════════════════════
class TwoLinePlateHandler:
    ...
    Example:  MH01BJ
              2121
    Triggered only when PlateLineDetector confirms a gap.
    """
    ...
    def read(bgr, easy_engine, paddle_engine):
        ...
        top_text, top_conf = TwoLinePlateHandler._read_half(
            top_crop, easy_engine, paddle_engine)
        bot_text, bot_conf = TwoLinePlateHandler._read_half(
            bot_crop, easy_engine, paddle_engine)
        ...
        if resolved and resolved != merged:
            candidates.append(
                (resolved, avg_conf + 0.05, 'two-line-resolved'))
    ...
    @staticmethod
    def _read_half(crop, easy_engine, paddle_engine):
        variants      = MultiPreprocessor.all_variants(crop)
        corr_variants = MultiPreprocessor.all_variants(
            PerspectiveCorrector.correct(crop))
        ...
        voted_text, voted_conf, _ = VotingSystem.merge(
            paddle_results, easy_results)
        return (voted_text, voted_conf) if voted_text else ('', 0.0)
...
class MultiPreprocessor:
    ...
    def all_variants(bgr, dirty_cleaned=None):
        ...
        # dirty variants — only added when cleaning was applied
        if dirty_cleaned is not None:
            ds  = MultiPreprocessor.standardise(dirty_cleaned)
            dg  = cv2.cvtColor(ds, cv2.COLOR_BGR2GRAY)
            dgc = cv2.createCLAHE(clipLimit=3.0,
                                   tileGridSize=(8, 8)).apply(dg)
            db  = cv2.GaussianBlur(dgc, (3, 3), 0)
            _, dotsu = cv2.threshold(
                db, 0, 255,
                cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            variants['dirty']        = ds
            variants['dirty_binary'] = cv2.cvtColor(dotsu,
                                           cv2.COLOR_GRAY2BGR)
            variants['dirty_grey']   = cv2.cvtColor(dgc,
                                           cv2.COLOR_GRAY2BGR)
...
class ANPRPipeline:
    ...
    def run(self, plate_bgr, is_camera=False):
        ...
        # Step 1.5 — Dirty detection
        dirty_score, dirty_severity, dirty_breakdown = \
            DirtyImageDetector.score(corrected)
        dirty_cleaned  = None
        cleaning_steps = []
        if dirty_severity != 'CLEAN':
            dirty_cleaned, cleaning_steps = \
                DirtyImageCleaner.clean(corrected, dirty_severity)

        # Step 2.5 — Route: single-line vs two-line
        if PlateLineDetector.is_two_line(corrected):
            print(f"  [PIPELINE] → Two-line route")
            return TwoLinePlateHandler.read(corrected, self.easy, self.paddle)
        else:
            print(f"  [PIPELINE] → Single-line route")

        # Step 4 — Multi-preprocess (single-line route)
...
class CustomANPR:
    ...
    def setup_ui(self):
        ...
        Label(
            hdr,
            text="Detection and Recognition Obscured Number Plates.",
            bg="#0a1628", fg="white",
    ...
    def _ready(self, paddle_ok):
        ...
        parts += ["Voting", "65-Pair", "Regex", "NightVision"]
        self.lbl_status.config(text="Detection and Recognition of Obscured Vehicle Number Plates", fg="#00E676")
    ...
    def run_yolo(self, arr):
        ...
        # ── Contour fallback when YOLO finds nothing ───────────
        fallback_crop = None
        if res is None or not any(len(r.boxes) > 0 for r in res):
            print(f"  [YOLO] No detection — trying contour fallback")
    ...
    def _display_vis(self, vis):
        """Resize and show the annotated frame on the canvas."""
    ...
    def _contour_fallback(self, bgr):
        """
        Traditional CV plate finder — runs only when YOLO finds nothing.
        Looks for rectangles with plate-like aspect ratio (1.5:1 to 7:1).
        Returns the best BGR crop or None.
        """
        h, w       = bgr.shape[:2]
        gray       = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
        candidates = []
        ...
        for blur_k in [5, 9]:
            ...
            for edges in [
                ...
                for c in sorted(cnts, key=cv2.contourArea,
                                 reverse=True)[:20]:
        ...
        if not candidates:
            print(f"  [FALLBACK] No contour candidates found")
            return None

        # Pick candidate closest to ideal 3:1 plate ratio
        best = min(candidates, key=lambda x: abs(x[2] - 3.0))
        print(f"  [FALLBACK] Crop found — "
              f"area={best[1]:.0f} ratio={best[2]:.2f}")
        # Return as RGB since rest of pipeline expects RGB
        return cv2.cvtColor(best[0], cv2.COLOR_BGR2RGB)
