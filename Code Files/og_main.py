"""
=================================================================
ANPR SYSTEM v12.0 — DIRTY IMAGE DETECTION EDITION
=================================================================
Pipeline (exact flowchart order):

  1.   Low Light Check    - auto-detect dark images
  1.5  Dirty Detection    - score plate: blur, contrast, colour,
                           edge density (NEW)
  1.6  Dirty Cleaning     - morph open + NLM denoise + CLAHE +
                           adaptive threshold (NEW)
  2.   YOLOv8             - detect plate (enhanced input if dark)
  3.   Perspective        - correct skew / tilt
  4.   Multi-Preprocess   - Plain/Grey/Binary/CLAHE/Sharp/Night/
                           Bloom/Dirty (NEW variant)
...
NEW IN v12.0:
  - DirtyImageDetector: composite score from 4 metrics
      * Laplacian variance  (sharpness / blur level)
      * Contrast std dev    (pixel spread)
      * Colour anomaly      (non-white/yellow hue dominance)
      * Edge density        (Canny edge pixel ratio)
  - Three severity levels: CLEAN / MILD / SEVERE
      * MILD  — light cleaning (NLM denoise + CLAHE)
      * SEVERE — full pipeline (morph open + NLM + CLAHE + adaptive)
  - DirtyImageCleaner: four-stage cleaning pipeline
  - New 'dirty' variant added to MultiPreprocessor
  - ANPRPipeline.run() updated: dirty check runs after perspective
    correction, cleaned plate fed into MultiPreprocessor
  - Detail output now reports dirty score and severity

INHERITED FROM v11.9:
...
# ══════════════════════════════════════════
# STEP 1.5 — DIRTY IMAGE DETECTOR (NEW)
# ══════════════════════════════════════════
class DirtyImageDetector:
    """
    Scores a plate crop across four independent metrics and returns
    a composite 0-1 dirtiness score plus a severity level.

    Metrics
    -------
    1. Laplacian variance  — low value = blurry / soft edges (mud/frost)
    2. Contrast std dev    — low value = flat, low-contrast image
    3. Colour anomaly      — high value = dominant non-white/yellow hue
    4. Edge density        — low value = few character edges visible

    Severity levels
    ---------------
    CLEAN   score < THRESHOLD_MILD    → no cleaning needed
    MILD    score < THRESHOLD_SEVERE  → light clean (denoise + CLAHE)
    SEVERE  score >= THRESHOLD_SEVERE → full clean pipeline
    """

    # Composite score thresholds (tune on your dataset)
    THRESHOLD_MILD   = 0.35
    THRESHOLD_SEVERE = 0.55

    # Per-metric weights (must sum to 1.0)
    W_LAPLACIAN = 0.30
    W_CONTRAST  = 0.20
    W_COLOUR    = 0.25
    W_EDGE      = 0.25

    # ── Metric calibration constants ─────────────────
    # Laplacian: values below this are considered blurry
    LAP_SHARP_MIN  = 500.0   # clean plate typical variance
    LAP_BLUR_MAX   = 50.0    # definitely blurry / dirty

    # Contrast: std dev of grey pixel intensities
    CONTRAST_GOOD  = 60.0    # clean plate typical std dev
    CONTRAST_POOR  = 15.0    # flat / occluded plate

    # Edge density: fraction of edge pixels after Canny
    EDGE_GOOD = 0.08         # enough character edges visible
    EDGE_POOR = 0.02         # almost no edges
    ...
    def score(bgr):
        ...
        Returns
        -------
        dirty_score : float  0.0 (clean) to 1.0 (very dirty)
        severity    : str    'CLEAN' | 'MILD' | 'SEVERE'
        breakdown   : dict   individual metric scores for debugging
        """
        gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)

        # ── Metric 1: Laplacian variance (sharpness) ──
        lap_var   = cv2.Laplacian(gray, cv2.CV_64F).var()
        # Invert and normalise: high variance = sharp = 0 dirt score
        lap_score = 1.0 - np.clip(
            (lap_var - DirtyImageDetector.LAP_BLUR_MAX) /
            max(DirtyImageDetector.LAP_SHARP_MIN - DirtyImageDetector.LAP_BLUR_MAX, 1),
            0.0, 1.0
        )

        # ── Metric 2: Contrast (std dev of grey intensities) ──
        std_dev      = float(np.std(gray))
        contrast_score = 1.0 - np.clip(
            (std_dev - DirtyImageDetector.CONTRAST_POOR) /
            max(DirtyImageDetector.CONTRAST_GOOD - DirtyImageDetector.CONTRAST_POOR, 1),
            0.0, 1.0
        )

        # ── Metric 3: Colour anomaly ──
        # White / yellow plate expected → detect dominant brown/grey hue
        hsv   = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(hsv)
        # High saturation + non-yellow hue signals mud/rust/frost
        dirt_mask = (
            (s > 30) &
            ~((h > 15) & (h < 35))   # yellow range excluded
        )
        colour_score = float(np.sum(dirt_mask)) / max(gray.size, 1)
        colour_score = np.clip(colour_score * 3.5, 0.0, 1.0)  # scale up

        # ── Metric 4: Edge density ──
        blur        = cv2.GaussianBlur(gray, (3, 3), 0)
        edges       = cv2.Canny(blur, 40, 120)
        edge_ratio  = float(np.sum(edges > 0)) / max(gray.size, 1)
        # Invert: low edge density = more dirty
        edge_score  = 1.0 - np.clip(
            (edge_ratio - DirtyImageDetector.EDGE_POOR) /
            max(DirtyImageDetector.EDGE_GOOD - DirtyImageDetector.EDGE_POOR, 0.001),
            0.0, 1.0
        )

        # ── Weighted composite ──
        composite = (
            DirtyImageDetector.W_LAPLACIAN * lap_score +
            DirtyImageDetector.W_CONTRAST  * contrast_score +
            DirtyImageDetector.W_COLOUR    * colour_score +
            DirtyImageDetector.W_EDGE      * edge_score
        )
        composite = float(np.clip(composite, 0.0, 1.0))

        if composite < DirtyImageDetector.THRESHOLD_MILD:
            severity = 'CLEAN'
        elif composite < DirtyImageDetector.THRESHOLD_SEVERE:
            severity = 'MILD'
        else:
            severity = 'SEVERE'

        breakdown = {
            'laplacian'  : round(lap_score, 3),
            'contrast'   : round(contrast_score, 3),
            'colour'     : round(colour_score, 3),
            'edge'       : round(edge_score, 3),
            'composite'  : round(composite, 3),
            'lap_var_raw': round(lap_var, 1),
            'std_raw'    : round(std_dev, 1),
            'edge_ratio' : round(edge_ratio, 4),
        }

        print(f"  [DIRTY] score={composite:.3f}  severity={severity}  "
              f"lap={lap_score:.2f} contrast={contrast_score:.2f} "
              f"colour={colour_score:.2f} edge={edge_score:.2f}")

        return composite, severity, breakdown


# ══════════════════════════════════════════
# STEP 1.6 — DIRTY IMAGE CLEANER (NEW)
# ══════════════════════════════════════════
class DirtyImageCleaner:
    """
    Four-stage cleaning pipeline applied based on severity.

    MILD severity  → stages 2 + 3  (denoise + CLAHE)
    SEVERE severity → all 4 stages  (morph open + denoise + CLAHE + adaptive)

    Stages
    ------
    1. Morphological opening  — removes small noise blobs (mud spots, frost dots)
                                kernel size scales with severity
    2. Non-local means denoise — smooths noise while preserving character edges
    3. CLAHE                  — lifts locally dark regions (mud patches) without
                                blowing out bright areas
    4. Adaptive thresholding  — binarises using local neighbourhood means,
                                handles uneven dirtiness across the plate
    """

    # NLM denoise strengths per severity
    NLM_H_MILD   = 8
    NLM_H_SEVERE = 14

    # CLAHE clip limits per severity
    CLAHE_CLIP_MILD   = 3.0
    CLAHE_CLIP_SEVERE = 4.5

    # Morphological kernel sizes
    MORPH_KERNEL_MILD   = 2
    MORPH_KERNEL_SEVERE = 3
    ...
    @staticmethod
    def clean(bgr, severity):
        """
        Parameters
        ----------
        bgr      : np.ndarray  plate crop in BGR
        severity : str         'CLEAN' | 'MILD' | 'SEVERE'

        Returns
        -------
        cleaned  : np.ndarray  cleaned BGR image
        steps    : list[str]   names of stages applied (for logging)
        """
        if severity == 'CLEAN':
            return bgr, []
        ...
        if severity == 'SEVERE':
            # Stage 1: Morphological opening — erode then dilate
            # removes small noise blobs while preserving character strokes
            k_size = DirtyImageCleaner.MORPH_KERNEL_SEVERE
            kernel = cv2.getStructuringElement(
                cv2.MORPH_RECT, (k_size, k_size))
            result = cv2.morphologyEx(result, cv2.MORPH_OPEN, kernel, iterations=1)
            steps.append('morph_open')

        # Stage 2: Non-local means denoising
        # compares patches across the image and averages similar ones
        # → removes random noise without blurring character edges
        h_val = (DirtyImageCleaner.NLM_H_SEVERE
                 if severity == 'SEVERE'
                 else DirtyImageCleaner.NLM_H_MILD)
        ...
        # applies contrast enhancement locally in tiles
        # → lifts dark muddy patches without over-brightening clean regions
        clip = (DirtyImageCleaner.CLAHE_CLIP_SEVERE
                if severity == 'SEVERE'
                else DirtyImageCleaner.CLAHE_CLIP_MILD)
        ...
        if severity == 'SEVERE':
            ...
            # uses local neighbourhood mean as threshold
            # → handles uneven dirtiness across the plate (one corner muddy, etc.)
            ...
            adapt  = cv2.adaptiveThreshold(
                ...
                DirtyImageCleaner.ADAPT_BLOCK,
                DirtyImageCleaner.ADAPT_C,
            )
            result = cv2.cvtColor(adapt, cv2.COLOR_GRAY2BGR)
            steps.append('adaptive_thresh')

        print(f"  [CLEAN] severity={severity}  steps={steps}")
        return result, steps
...
class MultiPreprocessor:
    ...
    @staticmethod
    def all_variants(bgr, dirty_cleaned=None):
        """
        Parameters
        ----------
        bgr            : np.ndarray  plate crop (perspective-corrected)
        dirty_cleaned  : np.ndarray | None
                         Pre-cleaned version from DirtyImageCleaner.
                         If provided, a 'dirty' variant is added so the
                         voting system can compare it against standard variants.
        """
        ...
        # NEW v12.0: add cleaned variant when dirty cleaning was applied
        if dirty_cleaned is not None:
            dirty_std = MultiPreprocessor.standardise(dirty_cleaned)
            dirty_grey = cv2.cvtColor(dirty_std, cv2.COLOR_BGR2GRAY)
            dirty_clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
            dirty_gc = dirty_clahe.apply(dirty_grey)
            dirty_blur = cv2.GaussianBlur(dirty_gc, (3, 3), 0)
            _, dirty_otsu = cv2.threshold(
                dirty_blur, 0, 255,
                cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            variants['dirty'] = dirty_std
            variants['dirty_binary'] = cv2.cvtColor(dirty_otsu, cv2.COLOR_GRAY2BGR)
            variants['dirty_grey'] = cv2.cvtColor(dirty_gc, cv2.COLOR_GRAY2BGR)
...
# ══════════════════════════════════════════
# TWO-LINE PLATE HANDLER (RESTORED v12.1)
# ══════════════════════════════════════════
class TwoLinePlateHandler:
    """
    Handles plates where text is split across two lines.
    Example:  MH01BJ
              2121
    Triggered only by aspect ratio (ph/pw > 0.45) — not content.
    """

    @staticmethod
    def read(bgr, easy_engine, paddle_engine):
        """
        Splits the plate crop horizontally into top and bottom halves,
        reads each half independently, then merges the results.
        """
        h, w = bgr.shape[:2]
        mid   = h // 2

        # Add small overlap so characters near the split are not cut
        overlap = max(int(h * 0.08), 6)
        top_crop = bgr[0 : mid + overlap, :]
        bot_crop = bgr[max(0, mid - overlap) : h, :]

        print(f"  [TWO-LINE] top={top_crop.shape} bot={bot_crop.shape}")

        top_text, top_conf = TwoLinePlateHandler._read_half(
            top_crop, easy_engine, paddle_engine, label='top')
        bot_text, bot_conf = TwoLinePlateHandler._read_half(
            bot_crop, easy_engine, paddle_engine, label='bot')

        print(f"  [TWO-LINE] top='{top_text}' ({top_conf:.1%})  "
              f"bot='{bot_text}' ({bot_conf:.1%})")

        merged = (top_text + bot_text).strip()
        if not merged or len(merged) < MIN_CHARS:
            return '', 0.0, 'two-line: no read'

        avg_conf = (top_conf + bot_conf) / 2.0

        # Run merged result through confusion resolver + regex
        resolved, corrections = ConfusionResolver.resolve(merged)
        fix_str = '; '.join(corrections) if corrections else 'none'

        candidates = [(merged, avg_conf, 'two-line-merged')]
        if resolved and resolved != merged:
            candidates.append((resolved, avg_conf + 0.05, 'two-line-resolved'))

        final_text, final_conf, final_src = RegexValidator.best_of(candidates)
        is_valid, _, fmt = RegexValidator.validate(final_text)

        detail = (
            f"Pipeline: two-line ({final_src})\n"
            f"Top: '{top_text}' ({top_conf:.1%})\n"
            f"Bot: '{bot_text}' ({bot_conf:.1%})\n"
            f"Merged: '{merged}'\n"
            f"Resolved: '{resolved}'\n"
            f"Fixes: {fix_str}\n"
    ...
    @staticmethod
    def _read_half(crop, easy_engine, paddle_engine, label):
        """Read a single half-crop using both OCR engines."""
        variants  = MultiPreprocessor.all_variants(crop)
        corrected = PerspectiveCorrector.correct(crop)
        corr_variants = MultiPreprocessor.all_variants(corrected)

        paddle_results = []
        easy_results   = []

        if paddle_engine and paddle_engine.loaded:
            paddle_results += paddle_engine.read(variants)
            paddle_results += paddle_engine.read(corr_variants)

        if easy_engine:
            easy_results += easy_engine.read(variants)
            easy_results += easy_engine.read(corr_variants)

        if not paddle_results and not easy_results:
            return '', 0.0

        voted_text, voted_conf, _ = VotingSystem.merge(
...
class ANPRPipeline:
    ...
    # ── NEW v12.1: strip embedded IND country label ──
    # Indian plates print "IND" on the left border.
    # When YOLO crop includes that edge, OCR reads it
    # into the result. Remove it unless it sits at
    # position 0-1 where a valid state code could be.
        corr = []
        if 'IND' in text[2:]:              # only strip if NOT at start
            cleaned = text.replace('IND', '', 1)
            if len(cleaned) >= MIN_CHARS:
                corr.append(f"stripped:IND from '{text}'")
                text = cleaned
    ...
    def run(self, plate_bgr, is_camera=False):
        ...
        # RE-ENABLED v12.1: trigger only on aspect ratio — much safer
        # A plate crop taller than 45% of its width is almost certainly
        # a two-line plate. Single-line plates never exceed this ratio.
        ph, pw = corrected.shape[:2]
        if pw > 0 and (ph / pw) > 0.45:
            print(f"  [TWO-LINE] Detected via ratio {ph/pw:.2f} — routing to two-line handler")
            return TwoLinePlateHandler.read(corrected, self.easy, self.paddle)
    
        # ── NEW v12.0: Dirty detection + cleaning (Steps 1.5 & 1.6) ──
        dirty_score, dirty_severity, dirty_breakdown = DirtyImageDetector.score(corrected)
        dirty_cleaned = None
        cleaning_steps = []
        if dirty_severity != 'CLEAN':
            dirty_cleaned, cleaning_steps = DirtyImageCleaner.clean(corrected, dirty_severity)

        # Build variants — pass cleaned image so 'dirty' variant is added
        variants      = MultiPreprocessor.all_variants(corrected, dirty_cleaned)
        ...
        is_valid, _, fmt = RegexValidator.validate(final_text)
        fix_str = '; '.join(corrections) if corrections else 'none'
        clean_str = ', '.join(cleaning_steps) if cleaning_steps else 'none'
        detail = (
            f"Pipeline: {final_src}\n"
            f"Restored: {fixes_str}\n"
            f"Dirty: score={dirty_breakdown['composite']:.3f} "
            f"severity={dirty_severity} "
            f"(lap={dirty_breakdown['laplacian']:.2f} "
            f"contrast={dirty_breakdown['contrast']:.2f} "
            f"colour={dirty_breakdown['colour']:.2f} "
            f"edge={dirty_breakdown['edge']:.2f})\n"
            f"Cleaning: {clean_str}\n"
...
class CustomANPR:
    ...
    def _contour_fallback(self, bgr):
    
        h, w = bgr.shape[:2]
        gray  = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)

        # Try multiple preprocessing to handle dirty/dark/damaged plates
        candidates = []

        for blur_k in [5, 9]:
            blurred = cv2.GaussianBlur(gray, (blur_k, blur_k), 0)

            # Both Canny and Otsu — different plates respond to different methods
            for edges in [
                cv2.Canny(blurred, 30, 120),
                cv2.Canny(blurred, 50, 200),
                cv2.threshold(blurred, 0, 255,
                    cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1],
            ]:
                # Dilate to close gaps in damaged/dirty plate borders
                ...
                for c in sorted(cnts, key=cv2.contourArea, reverse=True)[:20]:
                    ...
                    # Filter by aspect ratio — plates are always wider than tall
                    if ch == 0:
                        continue
                    ratio = cw / ch
                    ...
                    # Filter by area — not too small, not the whole image
                    area_ratio = (cw * ch) / (w * h)
                    if not (0.01 <= area_ratio <= 0.40):
                        continue
                    ...
                    candidates.append((crop, cv2.contourArea(c), ratio))

        if not candidates:
            return None

        # Pick the candidate with the most plate-like aspect ratio (3:1 target)
        best = min(candidates, key=lambda x: abs(x[2] - 3.0))
        print(f"  [FALLBACK] Contour crop found — "
              f"area={best[1]:.0f} ratio={best[2]:.2f}")
        return best[0]
    ...
    # ── Multi-scale retry (v12.1) ──────────────────────────────
    # If YOLO finds nothing at original size, retry at larger
    # scales. Small / angled / damaged plates are often missed
    # at normal resolution but detected when upscaled.
        SCALES = [1.0, 1.5, 2.0]
        res = None
        used_scale = 1.0
        ...
        for scale in SCALES:
            ...
            result = self.yolo(attempt, conf=0.20, verbose=False)

            # Check if any plate was detected
            detected = any(len(r.boxes) > 0 for r in result)
            if detected:
                res = result
                used_scale = scale
                print(f"  [YOLO] Plate found at scale {scale}x")
                break

        if res is None:
            # No detection at any scale — use original result as fallback
            res = self.yolo(arr, conf=0.20, verbose=False)
            print(f"  [YOLO] No plate detected at any scale")
        ...
        if res is None:
            print(f"  [YOLO] No plate detected — trying contour fallback")
            bgr_full = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
            fallback_crop = self._contour_fallback(bgr_full)
            if fallback_crop is not None:
                self.best_plate_crop = fallback_crop
                return   # skip rest of YOLO box processing
    ...
    def run_pipeline(self, tid):
        ...
        try:
            ...
            self._prog("Step 1.5: Dirty image detection...")
            self._step("1.5.DIRTY_DETECT")

            self._prog("Step 1.6: Dirty image cleaning...")
            self._step("1.6.DIRTY_CLEAN")
